...
if len(loss_values) >= window_size:
    trend_line = np.convolve(loss_values, np.ones(window_size)/window_size, mode='valid')
...
plt.plot(trend_epochs, trend_line, 'r-', linewidth=3, alpha=1, label=f'Trend (window={window_size})')
plt.xlabel('Epoch')
...
print(f"Minimum loss achieved at iteration {min_loss_idx + 1} (epoch {min_loss_epoch:.0f})")
