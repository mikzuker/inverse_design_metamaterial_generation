...
class Diffusion_model():
    ...
    def __sample__(self, batch_size: int, conditional_vec: torch.Tensor):
        ...
        sampled_seq = self.diffusion.p_sample_loop(shape = (batch_size, 1, 16), cond = conditional_vec)
    ...
    def __visualize_denoising_process__(self, shape: tuple, conditional_vec: torch.Tensor, specific_steps=None):
        self.__load__()
        denoising_steps = self.diffusion.visualize_denoising_process(shape = shape, cond = conditional_vec, specific_steps=specific_steps)
