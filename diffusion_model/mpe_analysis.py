...
def analyze_mpe_losses():
    ...
    for file_path in loss_files:
        ...
        print(f"Iteration {iteration} (Epoch {epoch:.0f}): Mean={stats['mean']:.2f}, Median={stats['median']:.2f}, Std={stats['std']:.2f}")
    ...
    epochs = [(step_index * samples_per_step) / dataset_size * 1000 for step_index in iterations]
    
    # Create the plot
    plt.figure(figsize=(12, 8))
    
    # Plot mean, median, and std data points with custom colors
    plt.plot(epochs, means, 'o-', label='Mean', linewidth=4, markersize=8, color='#FF6B6B', markerfacecolor='#FF6B6B', markeredgecolor='#CC5555', markeredgewidth=1.5)
    plt.plot(epochs, medians, 's-', label='Median', linewidth=4, markersize=8, color='#2ECC71', markerfacecolor='#2ECC71', markeredgecolor='#27AE60', markeredgewidth=1.5)
    plt.plot(epochs, stds, '^-', label='Standard Deviation', linewidth=4, markersize=8, color='#9B59B6', markerfacecolor='#9B59B6', markeredgecolor='#8E44AD', markeredgewidth=1.5)
