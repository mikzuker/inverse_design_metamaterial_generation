...
def reproduce_results(experiment_path, reproduce_dir):
        ...
        object_to_mimic = [
            particles.FiniteCylinder(
                ...
                refractive_index=complex(
                    obj["refractive_index"]["real"],
                    obj["refractive_index"]["imag"]
                ),
                euler_angles=obj["euler_angles"]
            ) if "height" in obj else
            particles.Sphere(
                radius=obj["radius"],
                position=obj["position"],
                refractive_index=complex(
    ...
    def plot_geometry():
        ...
        surface.__spheres_add__(coordinates_list=coordinates, spheres_radius_list=radiuses)
        surface.spheres_plot(save_path=reproduce_dir / 'spheres_surface_projection_reproduced.pdf')
        return surface
    ...
    def plot_spectrum(surface):
        ...
        angles_indices = np.round(np.degrees(hyperparameters["angeles_to_mimic"])).astype(int)
