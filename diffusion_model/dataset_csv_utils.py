...
def save_dataset_to_csv(experiment_dir: Path, 
                       angles: List[float], 
                       output_csv_path: Path,
                       dataset_type: str = "conditional") -> None:
    data_rows = []
...
def load_dataset_from_csv(csv_path: Path, 
                         angles: List[float],
                         dataset_type: str = "conditional") -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    for _, row in df.iterrows():
        ...
        if dataset_type == "conditional":
            ...
            features_tensor = features_tensor.view(1, 1, -1)  # (batch_size=1, channels=1, sequence_length)
            
            dscs_values = [row[col] for col in dscs_cols]
            conditions_tensor = torch.tensor(dscs_values, dtype=torch.float32)
            conditions_tensor = conditions_tensor.view(1, -1)  # (batch_size=1, num_angles)
...
def load_conditional_dataset_from_csv(csv_path: Path) -> Tuple[torch.Tensor, torch.Tensor]:
    df = pd.read_csv(csv_path)
    ...
    radius_cols = sorted([col for col in df.columns if col.startswith('radius_')], 
                        key=lambda x: int(x.split('_')[1]))
    dscs_cols = sorted([col for col in df.columns if col.startswith('dscs_angle_')], 
