...
class Dataset1D(Dataset):
    def __init__(self, tensor: Tensor, cond: Tensor):
        super().__init__()
    ...
    # def __len_tensor__(self):
    #     return len(self.tensor)

    # def __len_cond__(self):
    #     return len(self.cond)

    def __getitem__(self, idx):
        return (self.tensor[idx].clone(), self.cond[idx].clone())
...
class Block(Module):
    ...
    def forward(self, x, film_gamma = None, film_beta = None, scale_shift = None):
        x = self.proj(x)
        x = self.norm(x)
...
class ResnetBlock(Module):
    ...
    def forward(self, x, film_gamma = None, film_beta = None, time_emb = None):
        ...
        h = self.block1(x, film_gamma = film_gamma, film_beta = film_beta, scale_shift = scale_shift)

        h = self.block2(h, film_gamma = film_gamma, film_beta = film_beta)
...
class Unet1D(Module):
    def __init__(
        ...
        if self.use_film:
            assert cond_dim is not None, "You must specify `cond_dim` if using FiLM"
            self.num_film_layers = 2 * len(in_out) + 2 * len(in_out) + 2 + 1  # down, up, mid, final

            self.film_mlps = ModuleList([])
            film_hidden_dim = 128
    ...
    def forward(self, x, time, x_self_cond = None, cond = None):
        ...
        if self.use_film:
            assert cond is not None, "Condition vector must be provided when using FiLM"
            film_gammas = []
            film_betas = []
            ...
            for mlp in self.film_mlps:
                ...
                film_gammas.append(gamma)
                film_betas.append(beta)

            film_iter = iter(zip(film_gammas, film_betas))
            get_gamma_beta = lambda: next(film_iter)
        else:
            get_gamma_beta = lambda: (None, None)

        h = []

        for block1, block2, attn, downsample in self.downs:
            g, b = get_gamma_beta()
            x = block1(x, time_emb = t, film_gamma = g, film_beta = b)
            h.append(x)

            g, b = get_gamma_beta()
            x = block2(x, time_emb = t, film_gamma = g, film_beta = b)
            x = attn(x)
            h.append(x)
        ...
        g, b = get_gamma_beta()
        x = self.mid_block1(x, time_emb = t, film_gamma = g, film_beta = b)
        x = self.mid_attn(x)
        g, b = get_gamma_beta()
        x = self.mid_block2(x, time_emb = t, film_gamma = g, film_beta = b)
        ...
        g, b = get_gamma_beta()
        x = self.final_res_block(x, time_emb = t, film_gamma = g, film_beta = b)
...
class GaussianDiffusion1D(Module):
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, cond = None, x_self_cond = None, clip_denoised = True):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((b,), t, device = x.device, dtype = torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x = x, t = batched_times, cond = cond, x_self_cond = x_self_cond, clip_denoised = clip_denoised)
    ...
    def ddim_sample(self, shape, cond = None, clip_denoised = True):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            self_cond = x_start if self.self_condition else None
            pred_noise, x_start, *_ = self.model_predictions(img, time_cond, cond = cond, x_self_cond = self_cond, clip_x_start = clip_denoised)
...
class Trainer1D(object):
    ...
    def train(self, path: Optional[str] = None, loss_number: int = 0):
        accelerator = self.accelerator
        device = accelerator.device
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    if path is not None:
                        path_to_save_loss = Path(path)
                        path_to_save_loss.mkdir(exist_ok=True)
                        with open(path_to_save_loss / f'loss_{loss_number}.txt', 'a') as f:
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            all_samples_list = list(map(lambda n: self.ema.ema_model.sample(batch_size=n, cond = cond), batches))
